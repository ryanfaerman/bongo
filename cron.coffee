...
module.exports = new cron.CronJob '5 6,13,18,23 * * * *', () ->
	...
	if (queue.status is false) or (queue.recurrence.biweekly and week % 2) or (queue.recurrence.monthly and week / 4 is 0) then return else
		...
		if queue.day[day[today.getDay()]] and (queue.time.morning and today.getHours() is 6) or (queue.time.afternoon and today.getHours() is 11) or (queue.time.evening and today.getHours() is 18) or (queue.time.night and today.getHours() is 23)
			console.log "The time is now!"
			if week is 4 then week = 1 else week++
			
			console.log "This week is week #{week}"
			models.Episode.findOne({release: 'queue'}).sort('published', 'ascending').execFind (err, docs) ->
				episode = docs[0]
